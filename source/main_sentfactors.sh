...
training_folder=factorized_concat_sent_tok
...
dev_src_factors=$mt_dir/dev/$src.tok.tc.bpe${suffix}.TAGS.SENTAG
...
train_src_factors=$divergences_dir-base.heuristic.TAGS.SENTAG.$src
...
for i in $(seq 1 $run_n); do
	...
	. $scripts_dir/sockeye-train-transformer-factors.sh
done;
date;
